...
def main(args):
    ...
    if args.gradient_checkpointing:
        ...
        if (args.train_text_encoder or args.modifier_token is not None) and not args.use_lora:
        ...
        with TorchTracemalloc() as tracemalloc:
            for step, batch in enumerate(train_dataloader):
                with accelerator.accumulate(unet):
                    ...
                    if args.modifier_token is not None:
                        print('zeroing out grads for text encoder')
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step % args.save_steps == 0:
                        if accelerator.is_main_process:
                            print(f"Saving12 model checkpoint to {args.output_dir}")
    ...
    if accelerator.is_main_process:
        if args.use_lora:
            ...
            print('saving unet', os.path.join(args.output_dir, "unet"))
            unwarpped_unet.save_pretrained(
                os.path.join(args.output_dir, "unet"), state_dict=accelerator.get_state_dict(unet)
            )
            if args.train_text_encoder:
                print('saving text encoder', os.path.join(args.output_dir, "text_encoder"))
